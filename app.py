...
请从以下几个角度点评（中文，250-450字）：
1. 开场先写一句「同学你好，」（单独成句，后面接点评内容）；
2. 若对话记录中确有做得好之处，先具体指出；若确实没有，就如实说明这次问诊可以补充的地方，
   不要为了表扬而编造亮点；
3. 问诊质量：哪些关键信息还可以补充；
4. 诊断与治疗方案：与真实诊断的差距，以及可以怎么调整；
5. 学习建议：下一步重点看什么。

【必须严格遵守：只依据【问诊对话记录】】
- 点评中出现的一切「学生问了 / 学生做了 / 学生提到」的事实，必须能在上面的对话记录里逐条找到原话；
- 记录中没有出现过的提问、检查、判断，一律不得写成学生已经做过；需要建议时用将来语气，
  例如「还需要补充询问…」「下次可以追问…」，而不是「你追问了…」；
- 引用学生问话时保持与记录一致，不要改写成更专业的说法后再当作学生原话；
- 若对话记录很短或几乎没问到关键信息，就直接指出这一点（用商量的语气），不要虚构内容填充篇幅；
- 不确定的内容宁可不写。

【表达要求（重要）】
- 语气客气、尊重、鼓励为主，像带教老师私下交流，而不是打分裁判。
- 提不足时用商量与建设性措辞：「如果再加问一句…会更稳妥」「不妨考虑…」「这里还有提升空间」；
  避免「错误」「不对」「失误」「不合格」「太差」这类否定性判定，也不要反问质问。
- 建议不超过 3 条，避免堆砌缺点；不得评价学生本人（如「基础差」「不认真」）。
- 结尾给一句具体的鼓励或下一步行动。
直接给出点评文本，不要标题、不要编号列表、不要格式标记。"""

    try:
        from openai import OpenAI
        from config import LLM_API_KEY, LLM_BASE_URL, MEDICAL_MODEL, thinking_extra_param
        client = OpenAI(api_key=LLM_API_KEY, base_url=LLM_BASE_URL)
        # 开启思考：点评需要逐条核对"学生到底问过什么"，关闭思考时易凭印象编造提问记录。
        # max_tokens 给足（思考 token 也计入），否则正文可能为空。
        resp = client.chat.completions.create(
            model=MEDICAL_MODEL, temperature=0.3, max_tokens=2600,
            extra_body=thinking_extra_param(True, budget_tokens=1200),   # 限制思考预算，避免个别病例点评过慢
