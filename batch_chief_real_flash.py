...
def get_completed_set():
    """扫描已有 flash 结果文件，返回已完成 label 集合（续跑用）"""
    import glob as g
    completed = set()
    for f in g.glob(str(SAVE_DIR / "*flash*.json")):
        try:
...
def run_one(hadm_id: str, patient_ctx, complaint):
    label = f"{hadm_id}_Chief_Realistic_flash"
    try:
...
def main():
    print(f"Chief×Realistic flash 测试")
    ...
    for cid in CASE_LIST:
        ...
        label = f"{cid}_Chief_Realistic_flash"
        if label in skipped:
            skip_count += 1
            continue
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                t = futures[future]
                r = {"label": f"{t[0]}_Chief_Realistic_flash", "hadm_id": t[0],
    ...
    print(f"\n{'='*60}")
    print(f"Chief×Realistic flash 完成")
    ...
    summary_path = RESULTS_DIR / "batch_chief_real_flash_summary.json"
