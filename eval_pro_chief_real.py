...
OUT = '/opt/oral-mucosa-agent/outputs/logs/eval_pro_chief_real.txt'
lines = []

def p(s=''):
    lines.append(str(s))

files = sorted(glob.glob('/opt/oral-mucosa-agent/outputs/conversations/*_Chief_Realistic_pro_*.json'))
best = {}
for fn in files:
    try:
    ...
    if hadm not in TARGET:
        continue
    if not label.endswith('_Chief_Realistic_pro'):
        continue
    if hadm not in best:
        best[hadm] = (fn, d)
    else:
