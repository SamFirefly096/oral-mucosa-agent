#!/usr/bin/env python3
"""批量重跑所有24病例x6组合=144次实验 (fixed: hardcoded cases + config models)"""
...
from database import get_hpi_text, query_table, create_database
...
TARGET_CASES = [
    ...
    'MRAS001', 'OLL001', 'WSN001', 'CC001',
    # 2 科室真实病例
    'PIM001', 'MRS001',
]

print(f'使用模型: MEDICAL={MEDICAL_MODEL}, PATIENT={PATIENT_MODEL}')
print(f'目标病例: {len(TARGET_CASES)} 个')
...
# 只清理24个目标病例的旧6组对比文件（保留V036等其他test文件）
print('清理目标病例旧文件...')
old_count = 0
for fp in SAVE_DIR.glob('*.json'):
    ...
    if any(tag in name for tag in ['_Chief_', '_Lrn_', '_Txt_', '_Learn_', '_Textbook_']):
        ...
        for case_id in TARGET_CASES:
            if name.startswith(case_id + '_'):
                fp.unlink()
                old_count += 1
                break
print(f'已删除 {old_count} 个目标病例旧文件（V036等test文件已保留）')
...
for doc_name, pat_name, DocAgentClass, PatAgentClass in CONFIGS:
    ...
    for i, hadm_id in enumerate(cases, 1):
        ...
        try:
            med = DocAgentClass(model=MEDICAL_MODEL, thinking=False)
            pat = PatAgentClass(model=PATIENT_MODEL)
...
模型: MEDICAL={MEDICAL_MODEL}, PATIENT={PATIENT_MODEL}
日志: {log_file}
{"="*60}
'''
print(summary)
