#!/usr/bin/env python3
"""Parallel batch batch_A: 27 runs (3 Textbook_Original + 24 Textbook_Realistic)"""
...
from agents_enhanced import TextbookMedAgent, OriginalPatientAgent, RealisticPatientAgent, PatientContext
from conversation import run_conversation
from config import SAVE_DIR, MEDICAL_MODEL, PATIENT_MODEL

BATCHES = [
    ("Textbook", "Original", ["LEUK001","PIM001","MRS001"]),
    ("Textbook", "Realistic", ["OLP001","PV001","OC001","RAS001","HSV001","DLE001","LEUK001","EM001","ANUG001","LR001","ATOLP001","BP001","ROM001","LEUK002","PV002","HZ001","EM002","AOU001","MRAS001","OLL001","WSN001","CC001","PIM001","MRS001"]),
]

total = sum(len(cases) for _, _, cases in BATCHES)
ok = 0
fail = 0
t0_total = time.time()

for doc_name, pat_name, cases in BATCHES:
    if doc_name == 'Textbook':
        DocClass = TextbookMedAgent
    elif doc_name == 'Chief':
        DocClass = ChiefMedAgent
    else:
        raise ValueError(f'Unknown doc_name: {doc_name}')

    if pat_name == 'Original':
        PatClass = OriginalPatientAgent
    elif pat_name == 'Realistic':
        PatClass = RealisticPatientAgent
    else:
        raise ValueError(f'Unknown pat_name: {pat_name}')
...
elapsed_total = time.time() - t0_total
print(f'\n{"="*40}\nbatch_A DONE: {ok}/{total} ok, {fail} fail, {elapsed_total/60:.0f}min\n{"="*40}', flush=True)
